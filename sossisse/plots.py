...
def plot_sossisse(tbl, params):
    ...
    for i in range(nrows):
        ...
        ax[i].errorbar(np.arange(len(tbl))[oot], val[oot], yerr=errval[oot],
                       fmt='.', color='green', alpha=alpha, label='oot - valid')

        it = params['it_domain']
        ax[i].errorbar(np.arange(len(tbl))[it], val[it], yerr=errval[it],
                       fmt='.', color='orange', alpha=alpha, label='in transit')

        oot_not_valid = (~params['oot_domain'])*(~it)
        ax[i].errorbar(np.arange(len(tbl))[oot_not_valid], val[oot_not_valid], yerr=errval[oot_not_valid],
                       fmt='.', color='grey', alpha=alpha, label='oot - not valid')

...
from etienne_tools import printc, sigma, odd_ratio_mean, lin_mini_errors,art
...
def white_light_curve(param_file_or_params, force = False):
    ...
    print(art('White light curve '+params['object'],'blue','CYAN'))
    ...
    if params['recenter_trace_position']:
        printc('Scan to optimize position of trace','info')
        ...
        printc('Best dx : {} pix'.format(best_dx),'number')
        printc('Best dy : {} pix'.format(best_dy),'number')
    ...
    for i in tqdm(range(cube.shape[0]), leave=False):
        # find best combination of scale/dx/dy/rotation
        ...
        with warnings.catch_warnings(record=True) as _:
            amp0 = odd_ratio_mean(cube[i].ravel() / med.ravel() * mask.ravel(), err[i].ravel() / med.ravel())
        ...
        mask[bad] = np.nan
        amp_model, err_model, recon = lin_mini_errors(cube[i] * mask, err[i], v)
        ...
        tbl['rms_cube_recon'][i] = sigma(np.array(cube[i], dtype=float))
    ...
    if ['per_pixel_baseline_correction']:
        printc('Performing per-pixel baseline subtraction','info')
    ...
    for method in science.get_rms_baseline():
        ...
        printc('{0}, rms = {1:.1f}ppm'.format(method, rms1 * 1e6),'number')
...
def summary(obj, search_string='*'):
    ...
    for i in range(len(tbl)):
        ...
        if len(tmp) != 0:
            file_phot = tmp[0]
            printc('File with photometry info: {}'.format(file_phot), 'info')
            tbl_phot = Table.read(file_phot)
            amp = tbl_phot['amplitude']
            rms = sigma(amp - np.roll(amp, 1)) / np.sqrt(2) * 1e6
...
def spectral_extraction(param_file_or_params, force = False):
    ...
    print(art('Spectral timeseries '+params['object'],'blue','CYAN'))
    ...
    if force:
        printc('We have force = True as an input, we re-create temporary files if they exist.','bad3')
    ...
    for trace_order in params['trace_orders']:
        ...
        for nth_obs in tqdm(range(residual.shape[0]), leave = False):
            for spectral_bin in range( med.shape[1]):
                ...
                with warnings.catch_warnings(record=True) as _:
                    try:
                        ratio = odd_ratio_mean(v1/v0,v2/v0)
                    except:
                        ratio = np.nan, 0
        ...
        if 'reject_domain' in params:
            for i_reject in range(len(params['reject_domain'])//2):
                in_transit_reject[params['reject_domain'][i_reject*2]:params['reject_domain'][i_reject*2+1]] = True
        ...
        if params['remove_trend']:
            for i in range( sp.shape[1]):
                ...
                if np.sum(g) <2:
                    continue
                fit = np.polyfit(np.arange(sp.shape[0])[~in_transit_reject][g],v1[g],params['transit_baseline_polyord'])
        ...
        if params['saveresults']:
            ...
            printc('We write {}'.format(outname),'info')
            ...
            hdul.writeto("{}/spectra_ord{}{}.fits".format(params['FITS_PATH'], trace_order, params['tag2']), overwrite=True)
        ...
        with warnings.catch_warnings(record=True) as _:
            ...
            sp_out = np.nansum(sp[~in_transit_reject]*weight[~in_transit_reject],axis=0)/\
                     np.nansum(weight[~in_transit_reject],axis=0)
        ...
        if np.nanmin(wavegrid) !=0:
            ...
            for ibin in np.unique(wbin):
                g = wbin == ibin
                moy, yerr = odd_ratio_mean(sp_in[g],
                                           err_in[g])

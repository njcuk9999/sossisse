...
def white_light_curve(param_file_or_params, force=False):
    ...
    # we remove the principal components of the cube through a decorrelation of the behaviour of brighter pixels
    residu_pca = science.pca_clean_pix(cube, med, params)
    outname = '{}/recon_pixel_decorr_{}.fits'.format(params['TEMP_PATH'], params['tag'])
    soss_io.writeto(outname, residu_pca, overwrite=True)
    cube -= residu_pca
    ...
    # TODO: you shouldn't use '/'  as it is OS dependent
    # TODO: use os.path.join(1, 2, 3)
    outname = '{}/errormap_{}.fits'.format(params['TEMP_PATH'], params['tag'])
    if not os.path.isfile(outname):
        soss_io.writeto(outname, err, overwrite=True)

    # TODO: you shouldn't use '/'  as it is OS dependent
    # TODO: use os.path.join(1, 2, 3)
    outname = '{}/residual_{}.fits'.format(params['TEMP_PATH'], params['tag'])
    if not os.path.isfile(outname):
        soss_io.writeto(outname, cube, overwrite=True)

    # TODO: you shouldn't use '/'  as it is OS dependent
    # TODO: use os.path.join(1, 2, 3)
    outname = '{}/recon_{}.fits'.format(params['TEMP_PATH'], params['tag'])
    if not os.path.isfile(outname):
        misc.printc('Writing the reconstructed cube', 'info')
        soss_io.writeto(outname, all_recon, overwrite=True)

    tbl['oot-valid'] = params['oot_domain']
    tbl['in-transit-valid'] = params['it_domain']
    tbl['oot-before'] = params['oot_domain_before']
    tbl['oot-after'] = params['oot_domain_after']

    # TODO: you shouldn't use '/'  as it is OS dependent
    # TODO: use os.path.join(1, 2, 3)
    tbl.write('{}/soss_stability_{}.csv'.format(params['CSV_PATH'], params['tag']), overwrite=True)
    tbl.write('{}/soss_stability_{}.csv'.format(params['PLOT_PATH'], params['tag']), overwrite=True)
    ...
    plots.plot_sossisse(tbl, params)
...
def wrapper(param_file, force=False):
    ...
    outfile = '{}/soss_stability_{}.csv'.format(params['CSV_PATH'], params['tag'])
...
def spectral_extraction(param_file_or_params, force=False):
    ...
    for trace_order in params['trace_orders']:
        ...
        outname = os.path.join(params['CSV_PATH'], 'soss_stability_{}.csv'.format(params['tag']))
        tbl = Table.read(outname)

        # median trace after normalization
        outname = os.path.join(params['TEMP_PATH'], 'median_{}.fits'.format(params['tag']))
        med = soss_io.getdata(outname)

        # residual cube
        outname = os.path.join(params['TEMP_PATH'], 'residual_{}.fits'.format(params['tag']))
        ...
        # error cube
        filename = os.path.join(params['TEMP_PATH'], 'errormap_{}.fits'.format(params['tag']))
        err = soss_io.getdata(filename)

        # model trace to be compared
        filename = os.path.join(params['TEMP_PATH'], 'recon_{}.fits'.format(params['tag']))
        ...
        sp_odd = np.zeros([residual.shape[0], residual.shape[2]]) + np.nan  # with odd_ratio_mean
        sp_wavg = np.zeros([residual.shape[0], residual.shape[2]]) + np.nan  # with weighted average
        sp_sum = np.zeros([residual.shape[0], residual.shape[2]]) + np.nan  # with sum

        # placeholder for the N*2048 corresponding errors
        sp_odd_err = np.zeros([residual.shape[0], residual.shape[2]]) + np.nan
        sp_wavg_err = np.zeros([residual.shape[0], residual.shape[2]]) + np.nan
        sp_sum_err = np.zeros([residual.shape[0], residual.shape[2]]) + np.nan
        ...
        for nth_obs in tqdm(range(residual.shape[0]), leave=False):
            ...
            for spectral_bin in range(med.shape[1]):
                ...
                if ratio[1] != 0:
                    ...
                    sp_odd[nth_obs, spectral_bin] = ratio[0]
                    sp_odd_err[nth_obs, spectral_bin] = ratio[1]

                # with model adjustement
                with warnings.catch_warnings(record=True) as _:
                    try:
                        ratio = math.adjust_model(v1, v2, v0)
                    except:
                        ratio = np.nan, 0
                if ratio[1] != 0:
                    sp_wavg[nth_obs, spectral_bin] = ratio[0]
                    sp_wavg_err[nth_obs, spectral_bin] = ratio[1]

                # with sum
                with warnings.catch_warnings(record=True) as _:
                    try:
                        ratio = np.sum(v1) / np.sum(v0), np.sqrt(np.sum(v2 ** 2)) / np.sum(v0)
                    except:
                        ratio = np.nan, 0

                if ratio[1] != 0:
                    sp_sum[nth_obs, spectral_bin] = ratio[0]
                    sp_sum_err[nth_obs, spectral_bin] = ratio[1]

        # binary indexes of when the transit happens
        in_transit_reject = np.zeros(sp_odd.shape[0], dtype=bool)
        ...
        in_transit_integrate = np.zeros(sp_odd.shape[0], dtype=bool)
        in_transit_integrate[params['it'][1]:params['it'][2]] = True

        if params['remove_trend']:
            for i in range(sp_odd.shape[1]):
                v1 = sp_odd[~in_transit_reject, i]
                g = np.isfinite(v1)
                if np.sum(g) > 2:
                    fit = np.polyfit(np.arange(sp_odd.shape[0])[~in_transit_reject][g], v1[g],
                                     params['transit_baseline_polyord'])
                    sp_odd[:, i] -= np.polyval(fit, np.arange(sp_odd.shape[0]))

                v1 = sp_wavg[~in_transit_reject, i]
                g = np.isfinite(v1)
                if np.sum(g) > 2:
                    fit = np.polyfit(np.arange(sp_wavg.shape[0])[~in_transit_reject][g], v1[g],
                                     params['transit_baseline_polyord'])
                    sp_wavg[:, i] -= np.polyval(fit, np.arange(sp_wavg.shape[0]))

                v1 = sp_sum[~in_transit_reject, i]
                g = np.isfinite(v1)
                if np.sum(g) > 2:
                    fit = np.polyfit(np.arange(sp_sum.shape[0])[~in_transit_reject][g], v1[g],
                                     params['transit_baseline_polyord'])
                    sp_sum[:, i] -= np.polyval(fit, np.arange(sp_sum.shape[0]))
        ...
        if params['saveresults']:
            ...
            for sumtype in [0]:
                ...
                outname = os.path.join(params['FITS_PATH'],
                                       'residual_no_grey_ord{}_{}_{}.fits'.format(trace_order, params['tag2'],
                                                                                  sum_name))
                ...
                outname = os.path.join(params['FITS_PATH'],
                                       'residual_grey_ord{}_{}_{}.fits'.format(trace_order, params['tag2'],
                                                                               sum_name))
                misc.printc('We write {}'.format(outname), 'info')
                ...
                outname = os.path.join(params['FITS_PATH'],
                                       "spectra_ord{}_{}_{}.fits".format(trace_order, params['tag2'], sum_name))
                hdul.writeto(outname, overwrite=True)
        ...
        outname = os.path.join(params['FITS_PATH'],
                               'wavelength_ord{}_{}.fits'.format(trace_order, params['tag2']))
        misc.printc('We write {}'.format(outname), 'info')
        ...
        if params["saveresults"]:
            outname = os.path.join(params['CSV_PATH'],
                                   'tspec_ord{}_{}.csv'.format(trace_order, params['tag2']))
            np.savetxt(outname,

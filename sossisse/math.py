...
def adjust_model(data, error, model):
    # We find 'amp' that minimizes
    #   sum( (data - amp* model)**2/error**2)

    # We want to minimize
    #   sum( (data - amp* model)**2/error**2)
    #   sum( (data**2 - 2*amp*data*model + amp**2*model**2)/error**2)
    #   sum( data**2/error**2 - 2*amp*data*model/error**2 + amp**2*model**2/error**2) = sigma
    # damp/dsigma = 0
    #   -2*sum(data*model/error**2) + 2*amp*sum(model**2/error**2) = 0
    amp = sum(data*model/error**2)/sum(model**2/error**2)

    # we also want the error on amp considering the error on data and model
    err_amp = 1/np.sqrt(sum(model**2/error**2))

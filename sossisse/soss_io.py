...
def get_valid_oot(params):
    ...
    if 'reject_domain' in params.keys():
    ...
    params['it_domain'] = (np.arange(len(valid_oot)) > params['it'][0])*(np.arange(len(valid_oot)) < params['it'][3])
...
def getdata(file, header=False, ext=0):
    with fits.open(file) as hdul:
        ...
        if (type(data) == type(None)) and (ext == 0):
            data = hdul[1].data
...
def writeto(file, data, header=None, overwrite=True):
    misc.printc('Writing to {}'.format(file), 'info')
...
def sossisson_to_eureka(params):
    ...
    for trace_order in params['trace_orders']:
        ...
        # TODO: you shouldn't use '/'  as it is OS dependent
        # TODO: use os.path.join(1, 2, 3)
        fname = "{}/spectra_ord{}_{}_oddratio.fits".format(params['FITS_PATH'], trace_order, params['tag2'])  # "extract1d" specs
...
def clean_doublets():
    ...
    cs = []
    for file in tqdm(files, leave=False, desc='Computing checksums'):
        cs = np.append(cs, get_checksum(file))
    cs = np.array(cs)

    bad_file = cs == ''
    if np.sum(bad_file) > 0:
        misc.printc('We have {} files with no checksum, we remove them'.format(np.sum(bad_file)), 'bad1')
        files = files[~bad_file]
        cs = cs[~bad_file]

    for ucs in tqdm(np.unique(cs), leave=False, desc='Checking for duplicates'):
        g = np.where(cs == ucs)[0]
        if len(g) == 1:
            continue
...
def mk_tag(params):
    ...
    params['tag'] = params['object'] + '_' + params['suffix'] + '_' + params['checksum']
    ...
    # "None")
    params['tag2'] = params['object'] + '_' + params['suffix'] + '_' + params['checksum']
...
def load_yaml_params(param_file, force=False, do_time_link=False, silent=False):
    ...
    # we compile the size of all images
    sz = []
    ...
    params['DATA_X_SIZE'] = dim1
    params['DATA_Y_SIZE'] = dim2
    params['DATA_Z_SIZE'] = nframes

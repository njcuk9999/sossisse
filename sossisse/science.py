...
from wpca import PCA, EMPCA

from sossisse import math, misc
from sossisse import soss_io
from sossisse.soss_io import get_valid_oot
...
def get_effective_wavelength(params):
    # TODO: you shouldn't use '/'  as it is OS dependent
    # TODO: use os.path.join(1, 2, 3)
    med = soss_io.getdata('{}/median_{}.fits'.format(params['TEMP_PATH'], params['tag']))
...
def remove_background(cube, params):
    ...
    if params['bkgfile'] != '':
        ...
        if np.min(rms) / np.nanmedian(rms) > 0.99:
            misc.printc('We cannot conclude on the background shift, we keep dx=0', 'info')
            optimal_offset = 0.0
        else:
            imin = np.argmin(rms)
            ...
            if imin == 0 or imin == len(bgnd_shifts) - 1:
                ...
                misc.printc('Optimal background offset {:.3f} pix'.format(optimal_offset), 'number')
        ...
        if 'percentile_bgnd' not in params.keys():
            p_cut_bgnd = 25  # the same as masking >median and taking a median
        else:
            p_cut_bgnd = params['percentile_bgnd']  # input from yaml
            msg = f'We use the percentile {p_cut_bgnd} for the background'
            misc.printc(msg, 'info')

        if params['background_subtraction'] == 'same_lowp':
            med = np.nanmedian(cube, axis=0)
            lowp = math.lowpassfilter(np.nanpercentile(med, p_cut_bgnd, axis=0), 512)
            bg = np.tile(lowp, 256).reshape(moy.shape)

            for i in tqdm(range(cube.shape[0]), leave=False):
                cube[i] -= bg

        elif params['background_subtraction'] == 'different_lowp':
            for i in tqdm(range(cube.shape[0]), leave=False):
                with warnings.catch_warnings(record=True) as _:
                    lowp = math.lowpassfilter(np.nanpercentile(cube[i], p_cut_bgnd, axis=0), 512)

                cube[i] -= np.tile(lowp, 256).reshape(moy.shape)

        else:
            misc.printc('We do not subtract the background', 'info')
            stop
...
"""
def clean_1f_old(cube, err, params):
    ...
    if params['allow_temporary']:
        ...
        # TODO: you shouldn't use '/'  as it is OS dependent
        # TODO: use os.path.join(1, 2, 3)
        params['median_image_file'] = '{}/median_{}.fits'.format(params['TEMP_PATH'], params['tag'])
        params['clean_cube_file'] = '{}/cube_{}.fits'.format(params['TEMP_PATH'], params['tag'])
    ...
    if params['degree_1f_corr'] == 0:
        ...
        cube = fnoisemaker(cube, err, med_out, params)
...
def clean_1f(cube, err, params):
    ...
    residuals = np.zeros_like(cube)
    for i in tqdm(range(params['DATA_Z_SIZE']), leave=False, desc='1/f cleaning'):
        mask = np.isfinite(cube[i]) * np.isfinite(med) * params['TRACEMAP']
        amps[i] = np.nansum(cube[i][mask] * med[mask]) / np.nansum(med[mask] ** 2)

        # TODO ->> update in Neil's version of the code
        # TODO ->> one needs to differentiate between the binary and float masks
        mask_float = np.zeros_like(cube[i], dtype=float)
        mask[mask] = np.nan
        ...
        model_1f = np.zeros_like(tmp)

        for ite in range(3):
            diff = tmp - model_1f + mask_float

            with warnings.catch_warnings(record=True) as _:
                v1 = np.nanmedian(diff[:diff.shape[0] // 2:2], axis=0)
                v2 = np.nanmedian(diff[1:diff.shape[0] // 2:2], axis=0)
                v3 = np.nanmedian(diff[diff.shape[0] // 2::2], axis=0)
                v4 = np.nanmedian(diff[diff.shape[0] // 2 + 1::2], axis=0)
            zp_even = (v1 + v3) / 2
            slope_even = (v3 - v1) / 128
            zp_odd = (v2 + v4) / 2
            slope_odd = (v4 - v2) / 128

            index = np.arange(256) - 128

            for icol in range(model_1f.shape[1]):
                model_1f[::2, icol] += zp_even[icol] + slope_even[icol] * index[::2]
                model_1f[1::2, icol] += zp_odd[icol] + slope_odd[icol] * index[1::2]

        if False:
            p1, p99 = np.nanpercentile(tmp - model_1f, [5, 95])
            fig, ax = plt.subplots(nrows=2, ncols=1, sharex='all', sharey='all')
            ax[0].imshow(tmp, aspect='auto', vmin=p1, vmax=p99)
            ax[1].imshow(tmp - model_1f, aspect='auto', vmin=p1, vmax=p99)
            plt.show()
...
"""
def clean_1f_xxx(cube, params):
    This script is used to correct the 1/f noise in the NIRISS SOSS data. We fit
    a slope on odd and even pixels. This is done with medians to avoid outliers.
    # we must update the param file prior to checking if 1/f-cleaned files exist
    ...
    mask = np.zeros(med.shape, dtype=float)
    for i in range( med.shape[1]):
        with warnings.catch_warnings(record=True) as _:
            mask[:,i] = med[:,i]<np.nanpercentile(med[:,i], 90)
    mask[mask!=1.0] = np.nan

    for islice in tqdm(range(cube.shape[1]), leave=False, desc='1/f correction'):
        tmp = np.array(cube[islice])

        good = (np.isfinite(tmp) * np.isfinite(med) )
        amp = np.nansum(tmp[good] * med[good]) / np.nansum(med[good] ** 2)

        model_1f = np.zeros_like(tmp)

        for ite in range(3):
            diff = tmp - amp * med - model_1f + mask
            ...
            for i in range(model_1f.shape[1]):
                model_1f[::2,i] += zp_even[i] + slope_even[i] * index[::2]
                model_1f[1::2,i] += zp_odd[i] + slope_odd[i] * index[1::2]

        cube[islice] -= model_1f
...
def load_data_with_dq(params):
    ...
    if 'contamination' in params.keys():
        contamination_flag = True
        misc.printc('We read the contamination image', 'info')
        contam_file = os.path.join(params['CALIBPATH'], params['contamination'])
        contamination_image = soss_io.getdata(contam_file)
    ...
    for ifile in range(len(params['files'])):
        ...
        if contamination_flag:
            for iframe in tqdm(range(c.shape[0]), leave=False, desc='Subtracting contamination'):
                c[iframe] -= contamination_image * params['n_time_bin']
    ...
    if params['mode'] == 'SOSS':
        # we need to mask the edges of the SOSS data for the reference pixels
        cube[:, -4:, :] = np.nan
        cube[:, :, -4:] = np.nan
...
def fancy_centering(params, force=False):
    ...
    if force or not os.path.exists(outname):
        misc.printc('Creating {}'.format(outname), 'info')
        ...
        while dsig>0.01:
            ...
            dxdy_trace = curve_fit(pos_trace, np.arange(im.shape[1])[g],
                                   tracepos[g], p0=[0, 0])[0]
            ...
            dsig = np.abs(np.array( np.nanmax(nsig_res) - nsig_max))
        ...
        pos_dict = dict()
        for order in [1, 2]:
            # TODO -> this needs to be checked with Neil's version. The
            # TODO -> wavelength fit for ordre 1 was used in ordre 2
            # TODO -> Tables are read in reverse order to have the same order as the trace
            pos_dict[order] = Table.read(params['pos_file'], order)[::-1]

            spl_wavelength = ius(pos_dict[order]['X'] - dxdy_trace[0],
                                    pos_dict[order]['WAVELENGTH'])
            spl_throughput = ius(pos_dict[order]['X'] - dxdy_trace[0],
                                        pos_dict[order]['THROUGHPUT'])

            pos_dict[order]['X'] = pos_dict[order]['X'] - dxdy_trace[0]
            pos_dict[order]['WAVELENGTH'] = spl_wavelength(pos_dict[order]['X'])
            pos_dict[order]['THROUGHPUT'] = spl_throughput(pos_dict[order]['X'])

            pos_dict[order]['Y'] = pos_dict[order]['Y'] + dxdy_trace[1]
            pos_dict[order] = pos_dict[order][np.argsort(pos_dict[order]['X'])]
        ...
        spl_wave = ius(pos_dict[1]['X'], pos_dict[1]['WAVELENGTH'], k=3, ext=1)
        ...
        plt.plot(pos_dict[1]['X'], pos_dict[1]['Y'], 'g-')
        plt.plot(pos_dict[2]['X'], pos_dict[2]['Y'], 'r-')
        ...
        hdulist.append(fits.table_to_hdu(pos_dict[1]))
        hdulist.append(fits.table_to_hdu(pos_dict[2]))
...
def pca_clean_pix(residus, medtrace, params):
    if 'n_sample_pca_pixeldecorr' not in params:
        n_sample_pca_pixeldecorr = 1000
    else:
        n_sample_pca_pixeldecorr = params['n_sample_pca_pixeldecorr']
    ...
    # find brighter pixels on the trace to construct the sample for PCA computation
    peak = medtrace > np.nanpercentile(medtrace, 95)

    # reshape bright pixel map to a 2D array
    peak = peak.reshape(peak.shape[0] * peak.shape[1])
    # reshape residus to a 2D array
    sample = residus.reshape([residus.shape[0], residus.shape[1] * residus.shape[2]])
    # keep only bright pixels
    sample = sample[:, peak]

    # keep only sample n_sample_pca if there are more columns in sample
    if sample.shape[1] > n_sample_pca_pixeldecorr:
        np.random.seed(seed)
        sample = sample[:, np.random.choice(sample.shape[1],
                                            n_sample_pca_pixeldecorr, replace=False)]

    # remove median from each column for sample pixels
    for i in range(sample.shape[1]):
        sample[:, i] -= np.nanmedian(sample[:, i])

    # remove median from each row for sample pixels
    for i in range(sample.shape[0]):
        sample[i] -= np.nanmedian(sample[i])

    # remove NaNs and outliers
    sig = math.sigma(sample)
    sample[np.abs(sample / sig) > 5] = 0.0
    sample[np.isnan(sample)] = 0.0

    wpca = PCA()

    if 'n_pca_pix_decorr' not in params:
        wpca.n_components = 5
    else:
        wpca.n_components = params['n_pca_pix_decorr']

    wpca.fit(sample.T)
    components = wpca.components_

    recon = np.zeros_like(residus)

    for icol in tqdm(range(recon.shape[1]), desc='Reconstructing pix decorr'):
        for irow in range(recon.shape[2]):
            tmp = residus[:, icol, irow]
            keep = np.isfinite(tmp)
            if False in keep:
                tmp[~keep] = 0.0
            amps = np.dot(components, tmp)

            recon2 = np.zeros_like(tmp)
            for ipca in range(wpca.n_components):
                recon2 += amps[ipca] * components[ipca]
            recon[:, icol, irow] = recon2
